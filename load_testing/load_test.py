...
def process_args() -> Options:
    ...
    parser.add_argument('-f', '--file', metavar='FILE', type=str, default=DEFAULT_URLS_FILE, help='file with urls to parse')
    parser.add_argument('-s', '--scrapper', metavar='HOST', type=str, default=DEFAULT_SCRAPPER_HOST, help='scrapper host')
    parser.add_argument('-v', '--verbose', action='store_true', help='verbose output')
    parser.add_argument('-c', '--concur', metavar='N', type=int, default=DEFAULT_CONCUR_REQ, help='concurrent requests')
    args = parser.parse_args()

    if os.path.isfile(args.file) is False:
        parser.error(f'File {args.file} not found')
    if args.scrapper.startswith('http://') is False and args.scrapper.startswith('https://') is False:
...
def initial_report(pages: list[str], concur_req: int) -> None:
    ...
    if concur_req > 1:
        print(f'Parsing {len(pages)} page{plural} with {concur_req} concurrent requests.')
    else:
